    ...
    public class EditableTextBox
    {
        ...
        public void Draw(SpriteBatch b, Rectangle inputBounds, float phoneUiScale, bool isFocused)
        {
            ...
            if (IsMultiline)
            {
                ...
                // Scroll window calculations to keep cursor line visible
                int maxLines = Math.Max(1, (inputBounds.Height - (padding * 2)) / lineHeight);
                int startLine = 0;
                if (lines.Count > maxLines)
                {
                    startLine = Math.Clamp(cursorLineIndex - maxLines + 1, 0, lines.Count - maxLines);
                }
                int endLine = Math.Min(lines.Count, startLine + maxLines);
